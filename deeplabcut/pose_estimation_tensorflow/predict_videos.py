...
def analyze_videos(config,videos,videotype='avi',shuffle=1,trainingsetindex=0,gputouse=None,save_as_csv=False, destfolder=None,cropping=None,start=0,stop=1, network_path=None):
    """
    ...
    if not (network_path == None):
         dlc_cfg['init_weights'] = network_path
    else:
...
def AnalyzeVideo(video,DLCscorer,trainFraction,cfg,dlc_cfg,sess,inputs, outputs,pdindex,save_as_csv, destfolder=None,start=0,stop=1):
    ...
    try:
        ...
        if int(dlc_cfg["batch_size"])>1:
            if vid_type == 'fmf':
                PredicteData,nframes=GetPoseFfmf(cfg,dlc_cfg, sess, inputs, outputs,cap,nframes,int(dlc_cfg["batch_size"]),start=start,stop=stop)
            else:
